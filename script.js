/* ════════════════════════════════════════════════════════════
   IGOR ETO · 7077 · interações da landing page
...
(function () {
  ...
  (function navegacao() {
    ...
    const secoes = ['inicio', 'quem-e', 'caminho-recurso', 'trabalho-realizado', 'minha-regiao', 'participe']
  ...
  (function urna() {
    ...
    function digitar(valor) {
      if (digitos.length >= 4 || estado === 'confirmado') return;
      ...
      beep(1180, 0.07, 0.05);
      pintarCursor();
      if (digitos.length === 4) timers.push(setTimeout(finalizarNumero, 260));
    }
    ...
    function sequenciaAutomatica() {
      ...
      estado = 'digitando';
      dica.textContent = 'Digitando 7077…';
    ...
    function confirmar() {
      ...
      dica.textContent = 'Lado a lado com os mineiros · 7077';
      confete();
    ...
    teclado.addEventListener('click', (e) => {
      ...
      if (tecla.dataset.tecla) {
        ...
        if (estado === 'ocioso' || estado === 'digitando') {
          estado = 'digitando';
          dica.style.opacity = '1';
          dica.textContent = 'Digite os 4 números';
    ...
    function confete() {
      ...
      const cores = ['#1A3A8F', '#F5C400', '#FFE066', '#FFFFFF', '#2952CC'];
  ...
  (function ticker() {
    ...
    const grupo =
      `<span class="ticker-bloco tb-azul">${mg} Minas Gerais</span>` +
      `<span class="ticker-bloco tb-laranja">7077</span>` +
      `<span class="ticker-bloco tb-navy">IGOR ETO</span>` +
      `<span class="ticker-bloco tb-lima">AVANTE</span>` +
      `<span class="ticker-bloco tb-azul">${flor} O dinheiro do seu imposto de volta pra você</span>` +
      `<span class="ticker-bloco tb-navy">Lado a lado com os mineiros</span>`;
